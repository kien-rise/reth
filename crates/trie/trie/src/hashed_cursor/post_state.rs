...
pub struct HashedPostStateAccountCursor<'a, C> {
    ...
    post_state_cursor: ForwardInMemoryCursor<'a, B256, Account, FbBuildHasher<32>>,
    ...
    C: HashedCursor<Value = Account>,
{
    ...
    pub const fn new(cursor: C, post_state_accounts: &'a HashedAccountsSorted) -> Self {
        let post_state_cursor = ForwardInMemoryCursor::new(
            &post_state_accounts.accounts,
            &post_state_accounts.destroyed_accounts,
        );
...
pub struct HashedPostStateStorageCursor<'a, C> {
    ...
    post_state_cursor: Option<ForwardInMemoryCursor<'a, B256, U256, FbBuildHasher<32>>>,
    ...
    C: HashedStorageCursor<Value = U256>,
{
    ...
    pub fn new(cursor: C, post_state_storage: Option<&'a HashedStorageSorted>) -> Self {
        let post_state_cursor = post_state_storage
            .map(|s| ForwardInMemoryCursor::new(&s.non_zero_valued_slots, &s.zero_valued_slots));
    ...
    fn is_slot_zero_valued(&self, slot: &B256) -> bool {
        self.post_state_cursor.as_ref().is_some_and(|s| s.is_removed(slot))
    }
