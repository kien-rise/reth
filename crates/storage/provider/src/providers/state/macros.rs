    ...
    ($target:ty $(where [$($generics:tt)*])?) => {
        $crate::providers::state::macros::delegate_impls_to_as_ref!(
            ...
            StateProofProvider $(where [$($generics)*])? {
                ...
                fn witness(&self, input: reth_trie::TrieInput, target: std::sync::Arc<reth_trie::HashedPostState>) -> reth_storage_errors::provider::ProviderResult<alloy_primitives::map::B256HashMap<alloy_primitives::Bytes>>;
            }
            HashedPostStateProvider $(where [$($generics)*])? {
                fn hashed_post_state(&self, bundle_state: &revm::db::BundleState) -> std::sync::Arc<reth_trie::HashedPostState>;
            }
        );
    }
}
